...
parser.add_argument('-c',nargs=1,help='command (normally shell script)')
parser.add_argument('-a',nargs='*',help='input arguments',default=[])
parser.add_argument('-t',nargs=1,help='task id list as csv e.g. -t "1,2,3,4,5"')
parser.add_argument('-i',nargs=1,default=None,help='input file list(instead of task id list')
parser.add_argument('-f',nargs=1,default=None,help='filelist to read input from - only used with -i ')
...
if args.i is not None:
    print(args.f[0])
    filelist = pd.read_csv(args.f[0], sep = ' ',names=['id','path'],dtype={'id':str})
    id_list = args.i[0].split(',')
    id_list = [x.replace(' ','') for x in id_list]
    print(id_list)
    task_list = filelist[filelist['id'].isin(id_list)].index.astype('str').values
    print(task_list)
else:
    input_task_list = args.t[0]
